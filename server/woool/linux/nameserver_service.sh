...
#全局变量
###################################################################################
NAMESERVER_SERVICE=$WOOOL_ROOT/linux/nameserver_service.log
NAMESERVER_ERROR=$WOOOL_ROOT/linux/nameserver_error.log
###################################################################################
...
fn_main()
{
    ...
    if [  ! "${dir_path}" = "$WOOOL_ROOT/linux" ]
    then
        tms_fail "NameServer home directory error." || return $? 
    fi
    
    if [ ! -f $WOOOL_ROOT/linux/NameServer_d ]
    then
