...
function GotoRolePos(staticId)
	g_msgHandlerInst:registerMsgHandler(MASTER_SC_GET_POSITION_RET , function(buff)
		cclog("get MASTER_SC_GET_POSITION_RET")
...
function createRoleNode(school,cl_id,weapon_id,wing_id,scale,sex,touchCallBack,flagId)
...
-----------------------------------------------------------------------
	...
	if role_node then
		...
		if flagId and flagId > 0 then
			local w_resId = MpropOp.equipResId(flagId)
			local qit = createSprite(role_node,"res/showplist/qi/"..w_resId..".png",cc.p(14,40))
			qit:setLocalZOrder(-1)
			createEffect("qi_"..w_resId,cc.p(20,40),1237,180,2)
		end
		--]]
...
function useAndGoTo(protoId)
	cclog("useAndGoTo protoId = "..protoId)
	switch = {
		--寻宝符
		[5019] = function()
...
function __createAwardGroup( awards , isShowName , Interval , offX , isSwallow , addWidth )
	...
	cclog("iconNum:" .. tostring(#awards))
    for i = 1 , iconNum do
			cclog("iconID:" .. tostring(awards[i].id) .. ":bind:" .. tostring(awards[i].showBind))
		    local bind = false
			if awards[i].id ~= 1081 then
				bind = awards[i].showBind
			end
