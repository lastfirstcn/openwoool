...
local _gotoSwitchFun = {
    ...
    a37 = function()
		...
		--if  MPackManager:getPack(MPackStruct.eBag):countByProtoId(5993100)>0 then
		--	else
		--	TIPS( { type = 1 , str = "你的VIP称号不存在，无法进入高爆活动地图，请联系GM赞助后再来！ " } )			
	    --return
		--end
  ...
  --       if NewFunctionIsOpen(NF_FB_PROTECT) then
    ...
    a231 = function()
        ...
        if npcCfg then
            local tempMapID = tonumber(npcCfg.q_map)
            local mapInfo = getConfigItemByKey("MapInfo","q_map_id",tempMapID)
            local tempPos = cc.p(tonumber(npcCfg.q_x), tonumber(npcCfg.q_y))

            local feixieItemId = 1001
            local needItemId = tonumber(mapInfo.q_need_item_id or "1001")
            local minTitleId = tonumber(mapInfo.q_min_title_id or "0")
            local maxTitleId = tonumber(mapInfo.q_max_title_id or "0")
            local name = getConfigItemByKey("propCfg","q_id", needItemId,"q_name")
            local pack = MPackManager:getPack(MPackStruct.eBag)
            local num = pack:countByProtoId(needItemId)
            if num <= 0 then
                TIPS( { type = 1 , str = "你没有" .. name .. ", 无法传送! " } )
                return
            end
            local hasTitle = true
            if minTitleId > 0 and maxTitleId > 0 then
                local hasTitle = false
                local bank = MPackManager:getPack(MPackStruct.eBank)
                for i=minTitleId,maxTitleId,100 do
                    if pack:countByProtoId(i) > 0 or bank:countByProtoId(i) > 0 then
                        hasTitle = true
                        break
                    end
                end
            ...
            if hasTitle == false then
                TIPS( { type = 1 , str = "你没有达到进入" .. mapInfo.q_map_name .. "的条件, 无法传送! " } )
                return
            end

            local handlerFun = function()                                          
                require("src/layers/mission/MissionNetMsg"):sendClickNPC(npcId - 1)
            end
            --传送前往
            local TransmitFunc = function()
                local shoewNeedData = { 
                    targetData = { 
                        mapID = tempMapID , pos = {cc.p(tempPos.x+3,tempPos.y-3)}
                    }, 
                    noTipShop = false, 
                    q_done_event = 0,
                }
    ...
    a232 = function()
        __GotoTarget({ ru = "a231", Value = 13404})
    end,
    a233 = function()
        __GotoTarget({ ru = "a231", Value = 13405})
    end,
    a234 = function()
        __GotoTarget({ ru = "a231", Value = 13406})
    end,
    a235 = function()
        __GotoTarget({ ru = "a231", Value = 13407})
    end,
    a236 = function()
        __GotoTarget({ ru = "a231", Value = 13408})
    end,
    a237 = function()
        __GotoTarget({ ru = "a231", Value = 13409})
    end,
    a238 = function()
        __GotoTarget({ ru = "a231", Value = 13410})
    end,
    a239 = function()
        __GotoTarget({ ru = "a231", Value = 13411})
    end,
    a240 = function()
        __GotoTarget({ ru = "a231", Value = 13412})
    end,
    a241 = function()
        __GotoTarget({ ru = "a231", Value = 13413})
    end,
    a242 = function()
        __GotoTarget({ ru = "a231", Value = 13414})
    end,
    a243 = function()
        __GotoTarget({ ru = "a231", Value = 13415})
    end,
    a244 = function()
        __GotoTarget({ ru = "a231", Value = 13416})
    end,
    a245 = function()
        __GotoTarget({ ru = "a231", Value = 13417})
    end,
    a246 = function()
        __GotoTarget({ ru = "a231", Value = 13418})
    end,
    a247 = function()
        __GotoTarget({ ru = "a231", Value = 13419})
    end,
    a248 = function()
        __GotoTarget({ ru = "a231", Value = 13420})
    end,
    a249 = function()
        __GotoTarget({ ru = "a231", Value = 13421})
    end,
    a250 = function()
        --幸运抽奖
        local tmpData = require "src/layers/lottery/LotteryDB"
        if tmpData then
            for i=1, #tmpData do
                local data = tmpData[i]
                local temp = {}
                temp.itemID = data.objid
                temp.num = data.num
                temp.index = data.bind --顺序索引
                params[i] = temp
            end
    ...
    a251 = function()
        --积分兑换
        local acLayer = require("src/layers/lottery/ScoreCostLayer").new(params)
        getRunScene():addChild(acLayer,200)
    end,
    a252 = function()
        --尸王殿副本
			TransmitFunc(12001,80,50)
    end,
	 a255 = function()
        --灵犀仙岛
			TransmitFunc(12002,80,50)
    end,
	a256 = function()
	
	if MRoleStruct:getAttr(ROLE_LEVEL)<200 then
	TIPS( { type = 1 , str = "该地图大于200级才能进入，请提升等级再来。" } )	
	else
	TransmitFunc(12003,100,100)
	end
	...
	a257 = function()
		--连姻岛
	if MRoleStruct:getAttr(ROLE_LEVEL)<60 then	
	TIPS( { type = 1 , str = "该地图大于60级才能进入，请提升等级再来。" } )
	else
			TransmitFunc(12004,50,50)
	end	
	
    end,
	a258 = function()
	if MRoleStruct:getAttr(ROLE_LEVEL)<80 then	
	TIPS( { type = 1 , str = "该地图大于80级才能进入，请提升等级再来。" } )
	else
		--道门秘境
			TransmitFunc(12005,35,55)
	end
    end,
	a259 = function()
	if MRoleStruct:getAttr(ROLE_LEVEL)<100 then	
	TIPS( { type = 1 , str = "该地图大于100级才能进入，请提升等级再来。" } )
	else
		--幽冥鬼谷
			TransmitFunc(12006,20,30)
	end
    end,
	a260 = function()
	if MRoleStruct:getAttr(ROLE_LEVEL)<150 then	
	TIPS( { type = 1 , str = "该地图大于150级才能进入，请提升等级再来。" } )
	else
		--水晶冰宫
	TransmitFunc(12007,30,30)
	end
    end,
...
TransmitFunc = function(map,x,y)
                shoewNeedData = { 
                    targetData = { 
                        mapID = map , pos = {cc.p(x,y)}
                    }, 
                    noTipShop = false, 
                    q_done_event = 0,
                }
