...
function RichText:addText(str, defaultFontColor, isOutLine)
	...
	--cclog("createRichTextItem analyse end")
	--dump(self.data)
end
	...
	if infoTab[1] and infoTab[2] and infoTab[3] and infoTab[4] then
		...
		if mapName then
			...
			local func = function()
				...
				-- local feixieItemId = 1001 --只有飞鞋传送的地图才飞过去
        		-- local needItemId = tonumber(getConfigItemByKey("MapInfo","q_map_id",infoTab[1],"q_need_item_id"), feixieItemId) 
        		-- if not needItemId or needItemId == feixieItemId or needItemId <= 0 then
        		-- 	local detailMapNode = require("src/layers/map/DetailMapNode")
      			-- 	detailMapNode:goToMapPos(infoTab[1], cc.p(infoTab[3], infoTab[4]), false)
	            -- end
			end
...
function RichText:format()
	...
	while index <= #self.data do
		...
		--cclog("test index = "..tostring(index))
		--处理文字
