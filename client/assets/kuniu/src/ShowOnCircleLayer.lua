...
function ShowOnCircleLayer:createTouch()
	...
	listenner:registerScriptHandler(function(touch, event)
		...
		--cclog("EVENT_TOUCH_MOVED")
		local touchPos = touch:getLocation()
		--cclog("x="..touchPos.x.." y="..touchPos.y)
		if self.lastTouchPos == nil then
			...
			--cclog("self.angleFirst = "..self.angleFirst)
	...
	listenner:registerScriptHandler(function(touch, event)
		--cclog("EVENT_TOUCH_ENDED")
		--cclog("self.touchMove = "..self.touchMove)
...
function ShowOnCircleLayer:updateUI()
	for i,v in ipairs(self.param.nodes) do
		...
		-- if i == 6 then
		-- 	cclog("6 angleTemp == "..angleTemp)
		-- end
		-- cclog(i.." zorder = "..self.param.nodes[i].node:getLocalZOrder())
		self:updateEffect(i)
	end
	--cclog("self.angleFirst = "..self.angleFirst)
end
...
function ShowOnCircleLayer:setShowNodeByIndex(index)
	cclog("ShowOnCircleLayer:setShowNodeByIndex")
...
function ShowOnCircleLayer:stopAutoMove(isCheckShowFunc)
	cclog("stopAutoMove isCheckShowFunc = "..tostring(isCheckShowFunc))
