...
function RoleSprite:refreshData(params)
    ...
    if params and params[ROLE_SCHOOL] then
        local zhiye={"t_zh","t_fs","t_ds","t_lq"}
        local sex = {"_m","_f"}
...
function RoleSprite:getRightResID(e_type,w_resId)
    ...
    -- 衣服的特殊处理  模型缺少问题
    if e_type == PLAYER_EQUIP_UPPERBODY  then
        return RoleSprite:fixRoleResId(res_id)
    end
    return res_id
end

function RoleSprite:fixRoleResId(res_id)
    -- 衣服的特殊处理  模型缺少问题
    if res_id == 5120516 or res_id == 5110520 then
        print("---------------衣服的特殊处理  模型缺少问题----------------" .. res_id)
        return res_id + 100000
    end
	if  self.school == 4 then
	  return 80021413
     else
	 return res_id
	end
...
function RoleSprite:fixRoleEffResId(res_id)
    print("---------------衣服的特效特殊处理  模型缺少问题----------------" .. res_id)
    -- 衣服的特效特殊处理  模型缺少问题
    if res_id == 5020516 then
        return 5020515
    end
...
function RoleSprite:setEquipments(cl_id,weapon_id,wing_id)
    ...
    print("设置装备.............111111111111111......................")
    local MpropOp = require "src/config/propOp"
    local add_id = 0
    if self.up_ride then
        if not self.canRideWar then
            add_id = 1000  -- 骑乘
            -- 武器不显示
            self:setOnRide(true)
        else
    ...
    if wing_id and wing_id > 0 then
        ...
        if w_resId == nil and wing_id<=7 then
            w_resId= wing_id
        end
...
function RoleSprite:upOrDownRide_ex(role,ride_id,is_up,unsend,forceUpdate)
    ...
    if self ~= role then
        role.warRideId = ride_id
    end

    if (role.up_ride and role.ride_id and role.ride_id == ride_id and is_up) and forceUpdate ~= 1 then
        return
    end
    ...
    if  is_up then
        ...
        --[[取消自动攻击    delete by 396196516
        if self == role and role.warRideId and role.warRideId > 0 and game.getAutoStatus() == AUTO_ATTACK then
            game.setAutoStatus(0)
            G_MAINSCENE.hang_node:setOpacity(255)
            G_MAINSCENE.m_stopHangSpr:setVisible(false)
        end
        ]]
        ...
        if self == role and role.warRideId and role.warRideId > 0 and game.getAutoStatus() == AUTO_ATTACK then
            ...
            G_MAINSCENE.hangup_menu:setVisible(false)
        end
        ...
        -- 判断衣服和武器 能不能骑战 add by 396196516
        local canRideWar = RoleSprite:CanWarAttack(ride_id)
        role.canRideWar = canRideWar
        print("符合骑战：" .. tostring(canRideWar))
        -- 判断坐骑
        ...
        if w_resId then
            ...
            role:setIsOnHighRide(G_NO_OPEN_HIGHTRIDE)
            role:setIsOnAttackRide(not G_NO_OPEN_HIGHTRIDE)
            local ride = self:setEquipment_ex(role,PLAYER_EQUIP_RIDE,tostring(w_resId).."/down")
        end

        if not canRideWar then
            add_id = 1000  -- 骑乘
            -- 武器不显示
            role:setOnRide(true)
        else
    ...
    if self == role   then
        ...
        if eWeapon then
            ...
            print("武器1111 ID：" .. w_resId)
            if add_id == 2000 then
                w_resId = w_resId + 1000
            end
            print("武器2222 ID：" .. w_resId)
            local w_path = "weapon/" .. (w_resId)
        ...
        local clothId = MRoleStruct:getAttr(PLAYER_EQUIP_UPPERBODY,role:getTag())
        ...
        local w_resId = MPropOp.equipResId(clothId or 0)
        if w_resId <= 0 then w_resId = g_normal_close_id end
        w_resId = RoleSprite:getRightResID(PLAYER_EQUIP_UPPERBODY, w_resId+sex*100000 + add_id)
        if w_resId > 0 then
            local r_path = "role/" .. (w_resId)
...
function RoleSprite:upOrDownRide(is_up,no_action)
    ...
    if G_CONTROL:isFuncOn( GAME_SWITCH_ID_RIDE ) == false then
        is_up = false
    end

    if G_MAINSCENE.map_layer.isStory then
        is_up = G_MAINSCENE.storyNode:isCanUpRide()
    end
...
function RoleSprite:CanWarAttack(rideId,skillID)

    print("主动点击技能 开始是否能骑战............................")
    ...
    -- 判断衣服和武器 能不能骑战 add by 396196516
    local DonotWeapon = {2010101, 2020101, 2030101, 3010102, 3020102, 3030102, 4010103, 4020103, 4030103, 4010111, 4010115,4020111, 4030111}
    local DonotCloth = {2010501, 2011501, 2020501, 2021501, 2030501, 2031501, 3010502, 3011502, 3020502, 3021502, 3030502, 3031502,
                        4010503, 4011503, 4020503, 4021503, 4030503, 4031503, 4010510, 4011510, 4020510, 4021500, 4030510, 4031510}

    -- 只有马不让骑战  模型位置不对
    local DonotRide = {3001, 3002, 3003, 3004}
    if self.school == 1 and  skillID then
        --战士的2个骑战技能比较特殊 需要宠物动作
        if skillID == 1050 or skillID == 1051 then
            DonotRide = {3001, 3002, 3003, 3004, 3005,3006, 3014}
        end
    end

    print("判断灵兽是否能骑战:" .. tostring(ridePetId))
    if ridePetId then
        for i = 1, #DonotRide do
            if ridePetId == DonotRide[i] then
                print("灵兽不能骑战............................")
                return  false
            end
        end
    ...
    self.warRideId = ridePetId

    if eWeapon then
        for i = 1, #DonotWeapon do
            if eWeapon.mPropProtoId == DonotWeapon[i] then
                print("武器不能骑战............................")
                return  false
            end
        end
    ...
    if eClothing then
        for i = 1, #DonotCloth do
            if eClothing.mPropProtoId == DonotCloth[i] then
                print("衣服不能骑战............................")
                return false
            end
        end
...
function RoleSprite:setSkills(params)
    ...
    for k,v in pairs(params) do
        ...
        if jnfenlie == 1 or jnfenlie == 10 then
...
-- 监听装备发生变化
function RoleSprite:onChangeEquip()
    --更新角色状态
    RoleSprite:upOrDownRide_ex(self,self.ride_id,self.up_ride,true,1)
end
