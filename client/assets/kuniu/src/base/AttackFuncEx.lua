...
local isSafeAreaPlaySkill = function(skill_id)
	...
	local pass_skill_id_list = {1008,3010,2004,2009,3006,3008,3007,3003,3012,1054,2054,3054}
...
local roleStartToAttack = function(self,skillid,loop_time)
	...
	print("开始roleStartToAttack.................................")
	--骑战技能判断
	local Jnfenlie = getConfigItemByKey("SkillCfg","skillID",skillid,"jnfenlie")
	if Jnfenlie == 10 then

		-- 直接上马打
		if (not G_ROLE_MAIN:CanWarAttack(-1,skillid)) then
			self.skill_todo = {}
			TIPS{str="当前装备不能骑战", type=1, flag=1}
			resetGmainSceneTime()
			return false
		else
			if  not G_ROLE_MAIN.up_ride then
				print("..........................自动上马...............................")
		...
		--非骑战技能  不能骑战 先下马
		if G_RIDING_INFO and G_RIDING_INFO.state and G_RIDING_INFO.id and G_RIDING_INFO.id[1] then
			if not G_ROLE_MAIN:CanWarAttack() then
				--下马要发网络请求 所有回慢一点  导致技能先放了 这是一个不好的地方
				if  G_ROLE_MAIN.up_ride then
	...
	if MRoleStruct:getAttr(ROLE_SCHOOL) == 1 then
		...
		elseif skill_id == 1036 and (not has_lhbuff) then
			...
			return false
		elseif has_lhbuff and skill_id~=1005 and skill_id~=1035 and skill_id~=1010 and skill_id~=1038 and skill_id~=1008 and skill_id~=1050 then
	...
	if skill_info and skill_info.effectRangeType == 8 and target_node and target_node:isAlive() then --对自己释放
		...
		if do_attack and skill_id ~= 2004 then
			if CMagicCtrlMgr:getInstance():IsMagicCanDisplay(skill_id) == 3 then
                ...
                print("------------playSkillEffect-------333--------" .. tostring(skillid))
	...
	if m_tile_pos and isInRect(m_tile_pos,r_tile_pos,attack_R) or (skillid == 2003) then
		...
		if Jnfenlie == 10 then
			...
			print("---------------------------------骑战技能ID：" .. tostring(skillid))

			if skillid == 3050  or  skillid == 3051 then
			...
			print("------------doSkillAction---------------" .. tostring(skillid))
			G_MAINSCENE:doSkillAction(skillid)
			...
			if do_attack and skill_id ~= 2004 then
				if target_node and target_node:isAlive() then
					if CMagicCtrlMgr:getInstance():IsMagicCanDisplay(skill_id) == 3 then
                        ...
                        print("------------playSkillEffect-----111----------" .. tostring(skillid))
	...
	elseif m_tile_pos and isInRect(m_tile_pos,r_tile_pos,area_R) and (skill_id~=2004) and (skillid ~= 2003) then
		...
		if self.select_role and m_pos then
			...
			self.rock_action = performWithDelay(self.item_Node,function()
				...
				if m_loop_time == 6 then
					m_loop_time = 1
				end

				if self:roleStartToAttack(skill_id,m_loop_time) then --aleyre
					resetGmainSceneTime()
				end
			end,0.5)
		else
			...
			if do_attack then
				...
				if skill_id ~= 2004 then
					if CMagicCtrlMgr:getInstance():IsMagicCanDisplay(skill_id) == 3 then
                        ...
                        print("------------playSkillEffect-----222----------" .. tostring(skillid))
