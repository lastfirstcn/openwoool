...
function RoleSprite:setFaction_ex(role, factionId)
  ...
  local isSameFaction = function(factionId)
   -- log("my factionId = "..require("src/layers/role/RoleStruct"):getAttr(ROLE_FACTIONID))
    return (factionId == require("src/layers/role/RoleStruct"):getAttr(ROLE_FACTIONID) and factionId ~= 0)
  end
...
function RoleSprite:setFactionName_ex(role, factionName)
  ...
  self:showShaName(role, MRoleStruct:getAttr(ROLE_FACTIONID, role:getTag()))
  self:setSignPos()
  ...
  -- log("updateCornerSign_ex c_type = "..c_type)
   if G_MAINSCENE.map_layer and G_MAINSCENE.map_layer.role_tab then
    for k,v in pairs(G_MAINSCENE.map_layer.role_tab) do
      ...
      --  log("test 2")
        role_flag = require("src/layers/role/RoleStruct"):getAttr(ROLE_FACTIONID, v)
      end
...
function RoleSprite:setCornerSign_ex(role,c_type,flag)
  ...
  elseif c_type == 2 then -- 同会 -- 同盟
    local my_facid = require("src/layers/role/RoleStruct"):getAttr(ROLE_FACTIONID)
