...
local buffswitch = {
    ...
    [131] = function(value, objId, role_item)
        if value then
            cclog("[Exec update buffer by id 131] value true.")
        else
            cclog("[Exec update buffer by id 131] value false.")
        end
    ...
    [134] = function(value, objId, role_item)
        if value then
            cclog("[Exec update buffer by id 134] value true.")
            ...
            cclog("[Exec update buffer by id 134] value false.")
            role_item:standed()
...
function MainMapLayer:onEntityHurt(entity, objId, hurt, cur_blood, ishit, resist_type)
	...
	cclog("[MainMapLayer:onEntityHurt] called. objId = %d, hurt = %d, cur_blood = %d, resist_type = %d.", objId, hurt, cur_blood, resist_type)
	...
	for k, v in pairs(buffs) do
		...
		cclog("[MainMapLayer:onEntityHurt] lx = %d.", buff_hurttype)
		if buff_hurttype == 129 then	-- magic immunity
			if resist_type == 2 then	-- magic resist
				cclog("[MainMapLayer:onEntityHurt] magic resist.")
