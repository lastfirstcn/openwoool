...
function BaseMapScene:playHangupEffect(flag)
	...
	if flag == 0 then
		...
		-- 符合骑战条件判断  add by 396196516
		if not self.storyNode then
			if G_ROLE_MAIN:CanWarAttack() then
				print("自动攻击符合骑战条件........................")
...
function BaseMapScene:createMapLayer(map_str,mapId,pos,isfb)
	...
	cclog("BaseMapScene createMapLayer:" .. map_str)
	if self.map_layer then
...
function BaseMapScene:createNewFunctionBtn(record)
	--cclog("BaseMapScene:createNewFunctionBtn")
...
function BaseMapScene:removeNewFunctionBtn()
	--cclog("BaseMapScene:removeNewFunctionBtn")
...
--[[ 旧目标奖励
	--cclog("BaseMapScene:createNewFunctionBtnEx")
...
function BaseMapScene:createTargetAwards(update)	
	...
	local iconFun = function(sender,masking)
		if sender and masking then	
			...
			cc.CallFunc:create(function() createFlyParticle(startPos, endPos) end)))--function() cclog("123") end cc.CallFunc:create(createFlyParticle(startPos, pos) cc.MoveBy:create(1, cc.p(10, 10)))
...
function BaseMapScene:removeNewFunctionBtnEx()
	--cclog("BaseMapScene:removeNewFunctionBtnEx")
...
function BaseMapScene:isCanRemoveNewFunctionBtnEx()
	--cclog("BaseMapScene:isCanRemoveNewFunctionBtnEx")
	if self.newFuntionNodeEx then
...
function BaseMapScene:enterStoryMode(mapid)
	...
	cclog("BaseMapScene:enterStoryMode  mapID:" .. mapid)
...
function BaseMapScene:exitStoryMode()
	cclog("BaseMapScene:exitStoryMode")
    if self.storyNode then
...
function BaseMapScene:enterStoryExMode()
	--cclog("BaseMapScene:enterStoryExMode 11111111111111111111111111111111111111111")
	cclog("BaseMapScene:enterStoryExMode")
...
function BaseMapScene:exitStoryExMode()
	--cclog("BaseMapScene:enterStoryExMode 22222222222222222222222222222222222222222")
	cclog("BaseMapScene:exitStoryExMode")
...
function BaseMapScene:enterFactionFb(objId)
	cclog("BaseMapScene:enterFactionFb.................")
...
function BaseMapScene:addHangUpCheck(isForTuto)
	...
	local checkHangUpFunc = function()
		--cclog("checkHangUpFunc")
		if self.hangUpCheckTuto == true then
			return
		end
		--cclog("checkHangUpFunc 1")
		...
		if lv > 1 and lv < checkMaxLevel then
			--cclog("checkHangUpFunc 3 status = "..game.getAutoStatus())
			if game.getAutoStatus() == 0 and G_TUTO_NODE.touchNodeList and G_TUTO_NODE.touchNodeList[TOUCH_MAIN_TASK_GUIDE] then
				--cclog("checkHangUpFunc 4")
				tutoAddTipOnNode(G_TUTO_NODE.touchNodeList[TOUCH_MAIN_TASK_GUIDE], nil, nil, TOUCH_MAIN_TASK_GUIDE, nil, nil, function() 
						--cclog("check game.getAutoStatus() = "..game.getAutoStatus())
						return game.getAutoStatus() ~= 0
					end)
...
function BaseMapScene:setEquip()
	...
	-- 延迟 更换模型 add by 396196516
	if not self.storyNode then
		performWithDelay( self , function()
			G_ROLE_MAIN:onChangeEquip()
		end , 1.6 )
	end
...
function BaseMapScene:createBaseButton()
	cclog("[BaseMapScene:createBaseButton] called.")
