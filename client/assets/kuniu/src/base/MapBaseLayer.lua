...
function MapBaseLayer:initialize(r_pos)
			...
			elseif flag < 5 and G_ROLE_MAIN then
				...
				if get_good then
					if not (self.has_send_pickup and self.has_send_pickup == get_good) then
						...
						-- 拾取物品
						cclog("开始拾取物品：" .. tostring(get_good) .. ".........................." )
	...
	AudioEnginer.randMonsterMus(attacker:getMonsterId(),1)
	

	AudioEnginer.playEffect("sounds/monsterMusic/insect/hurt.mp4", false)
...
function MapBaseLayer:makeMainRole(px,py,filepath,maxcut,isMe,objid,entity)
	...
	cclog("MapBaseLayer makeMainRole 创建角色 id:" .. objid)
	cclog("MapBaseLayer makeMainRole 创建角色 isMe:" .. tostring(isMe))
	if entity[ROLE_NAME] then
		cclog("MapBaseLayer makeMainRole 创建角色 name:" .. entity[ROLE_NAME])
	end
	...
	if isMe then
		...
		if filepath then
			...
			-- 清除
			cclog("MapBaseLayer resetItemData清除")
			self:resetItemData()
	...
	if retok then
		...
		elseif role_model == 9003 then
			...
			if role_model == 9001 then
				cclog("entity[ROLE_MODEL]" .. role_model)
...
--     		local showBlood = function()
		...
		-- 	cclog("[MapBaseLayer:addMonster] called. entity id = %s, model id = %s, ROLE_SHOW = %s.", objid, role_model, entity[ROLE_SHOW])
			...
			local playFunc = function()
				...
				if do_attack then
                    ...
                    if (not self.isStory) and (CMagicCtrlMgr:getInstance():IsMagicCanDisplay(skill_id) == 3) then
						...
						print("-----MapBaseLayer-------playSkillEffect-----111----------" .. tostring(skill_id))
