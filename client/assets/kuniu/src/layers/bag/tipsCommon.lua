...
local tAction = {
	...
	use = function(params)
		...
		if girdId then
			...
			elseif protoId == 5007  or protoId == 5008 or protoId == 5009 then
				--判断如果已经学习了一个元神技能就不能再使用了
				local canLearn = true
				for k,v in ipairs(G_ROLE_MAIN.skills)do
					cclog("已学技能：" .. v[1])
					if v[1] == 1054 or v[1] == 2054 or v[1] == 3054 then
						cclog("已经学习了一个元神，不能再学习了")
						canLearn = false
					end
				end
				...
				if canLearn then
					...
					TIPS({ type = 1  , str = "已经学习了一个元神，不能再学习了" , isMustShow = true })
				end
			else
				MPackManager:useByGirdId(girdId)
			end
...
function tipsCommon:ctor(params)
	...
	if not isSpecialItem(protoId) then
		...
		local priceValue = MpropOp.recyclePrice(protoId)
		local priceIcon = "res/group/currency/1.png"
		if priceValue < 0 then
			priceIcon = "res/group/currency/3.png"
			...
			local build_outputWayNode = function()
				...
				for i, v in ipairs(way) do
					...
					if finx then
						...
						cell.status = status
						cell.gotoRecord = MPropOutput:gotoRecord(record)
									...
									if cc.pGetDistance(startPos,currPos) < 30 then
										...
										if cell.status then
											...
											performWithDelay(root, function()
												...
												__GotoTarget({ ru = cell.gotoRecord, protoId = protoId })
											end, 0.0)
										else
											if cell.msg then
