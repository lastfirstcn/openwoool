...
function FacApplyListLayer:networkHander(buff,msgid)
	local switch = {
		[FACTION_SC_AGREE_JOIN_RET] = function()    
			cclog("get FACTION_SC_AGREE_JOIN_RET"..msgid)
		...
		[FACTION_SC_REFUSE_APPLY_RET] = function()    
			cclog("get FACTION_SC_REFUSE_APPLY_RET"..msgid)
		...
		[FACTION_SC_GETAPPLYINFO_RET] = function()    
			cclog("get FACTION_SC_GETAPPLYINFO_RET"..msgid)
			...
			for i=1,num do 
				...
				for k,v in ipairs(self.load_data[i])do 
					cclog("self.load_data"..i.."k"..k.."v"..tostring(v))
				end
			end	
