...
function FactionFBNode:updateRankAll()
	cclog("[FactionFBNode:updateRankAll]")
...
function FactionFBNode:updateRank()
	cclog("FactionFBNode:updateRank")
...
function FactionFBNode:networkHander(buff,msgid)
	...
	[FACTIONCOPY_SC_GET_PASS_TIME_RET] = function ()
		...
		cclog("[FACTIONCOPY_SC_GET_PASS_TIME_RET] FactionFBNode time = %s, copyid = %s, setcount = %s.", self.bossTime, self.copyId, self.copySetCount)
...
--		end
	...
	[FACTIONCOPY_SC_OVER] = function ( )
		...
		self:updateTime(60)
		cclog("[FACTIONCOPY_SC_OVER] enter called.")
		...
		if notify == 1 then
			local funcD = function()
				...
				cclog("[FACTIONCOPY_SC_OVER] called.")
			end
