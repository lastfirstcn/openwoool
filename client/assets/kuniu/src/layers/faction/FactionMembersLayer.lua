...
function FactionMembersLayer:tableCellTouched(table,cell)
	...
	if self:isVisible() then
		...
		if data[1] ~= userInfo.currRoleStaticId then
			cclog("data[1]"..data[1].."G_ROLE_MAIN.obj_id "..G_ROLE_MAIN.obj_id )
...
function FactionMembersLayer:tableCellAtIndex(table, idx)
    ...
    if data[3] and data[8] then 
  		cclog("factionMembersLayer:name = "..data[3].." lastLoadTimeSec=".. data[8].." lastLoadTime="..formatDateTimeStr(data[8]) .." clientTimeSec="..GetTime().." " .."clientTime="..formatDateTimeStr(GetTime()).. " getStrByTime="..self:getStrByTime(data[8]))
    else
    	cclog("factionMembersLayer , nil" )
    end  
...
function FactionMembersLayer:networkHander(buff,msgid)
	local switch = {
		[FACTION_SC_GETALLMEMBER_RET] = function()    
			cclog("get FACTION_SC_GETALLMEMBER_RET"..msgid)
			self.data = {} 
			...
			for i=1,num do 
				...
				for k,v in ipairs(self.data[i])do 
					cclog("self.data"..i.."k"..k.."v"..tostring(v))
				end
			end	
		...
		[FACTION_SC_APPOINTPOSITION_RET] = function()    
			cclog("get FACTION_SC_APPOINTPOSITION_RET"..msgid)
			...
			elseif data[2] == userInfo.currRoleStaticId then 
				cclog("222222222222222222222222222")
		...
		[FACTION_SC_REMOVEMEMBER_RET] = function()    
			cclog("get FACTION_SC_REMOVEMEMBER_RET"..msgid)
