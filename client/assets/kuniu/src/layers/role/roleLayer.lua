...
local buildBeauty = function()
		...
		-- end
		local emptyBg=createSprite(right_bg, "res/common/bg/bg98.png", cc.p(right_bg_size.width/2, right_bg_size.height/2), cc.p(0.5, 0.5))
		emptyBg:setTag(1)
		createLabel(emptyBg, string.format("元神装备，敬请期待",40),cc.p(emptyBg:getContentSize().width/2,28), cc.p(0.5,0.5), 22, true,0, nil,MColor.lable_yellow)
end


local buildFabao = function()
		right_bg:removeChildByTag(1)
		right_bg:removeChildByTag(2)
		-- if G_WING_INFO.id == nil then
		-- 	MessageBox(string.format(game.getStrByKey("wr_wing_noWingTip"),getConfigItemByKey("NewFunctionCfg", "q_ID", 2).q_level), "")
		-- end
		local emptyBg=createSprite(right_bg, "res/common/bg/zzbg.png", cc.p(right_bg_size.width/2, right_bg_size.height/2), cc.p(0.5, 0.5))
		emptyBg:setTag(1)
		createLabel(emptyBg, string.format("法宝镶嵌，敬请期待",40),cc.p(emptyBg:getContentSize().width/2,28), cc.p(0.5,0.5), 22, true,0, nil,MColor.lable_yellow)
end
...
G_TUTO_NODE:setTouchNode(TabControl:tabAtTitle(meiren), TOUCH_ROLE_MEIREN)
G_TUTO_NODE:setTouchNode(TabControl:tabAtTitle(fabao), TOUCH_ROLE_FABAO)
