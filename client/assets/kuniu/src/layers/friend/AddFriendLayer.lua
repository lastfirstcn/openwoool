	...
	local switch = {
		[RELATION_SC_ADDRELATION_RET] = function() 
			...
			if e_code >= 0 then 
				...
				cclog("is_loaddata = "..tostring(is_loaddata))
				if is_loaddata then
		...
		[RELATION_SC_RECOMMEND_FRIEND_RET] = function() 
			cclog("get RELATION_SC_RECOMMEND_FRIEND_RET"..msgid)
