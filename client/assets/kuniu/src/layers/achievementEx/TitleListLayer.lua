...
function TitleListLayer:initData()
	...
	self.sex = require("src/layers/role/RoleStruct"):getAttr(PLAYER_SEX)
...
function TitleListLayer:updateRight(record)
	...
	if record.finish == true then
		...
		-- 	--cclog("record.leftTime = "..record.leftTime)
	...
	if data.q_sp then
		...
		for i,v in ipairs(strTab) do
			local richText = require("src/RichText").new(flag, cc.p(flag:getContentSize().width/2 , y), cc.size(150, 20), cc.p(0.5, 1), 20, 18, color)
			richText:setAutoWidth()
...
function TitleListLayer:tableCellTouched(table, cell)
	...
	cclog("self.lastSelectIdx = "..index)
...
function TitleListLayer:networkHander(buff,msgid)
	local switch = {
		[ACHIEVE_SC_GETTITLEDATARET] = function()
			cclog("get ACHIEVE_SC_GETTITLEDATARET")
		...
		[ACHIEVE_SC_SETTITLERET] = function()
			cclog("get ACHIEVE_SC_SETTITLERET")
		...
		[ACHIEVE_SC_DISLOADTITLERET] = function()
			cclog("get ACHIEVE_SC_DISLOADTITLERET")
		...
		[ACHIEVE_SC_GETACHIEVEDATARET] = function()
			cclog("get ACHIEVE_SC_GETACHIEVEDATARET")
