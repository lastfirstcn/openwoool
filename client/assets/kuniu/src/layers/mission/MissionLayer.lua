...
function M:portalGo( tempData , isNoPlot , isTask ,no_remove )
    ...
    if tempData.q_done_event and tonumber( tempData.q_done_event ) == 0  or isNoPlot then
        ...
        local needItemId = feixieItemId
        local mapInfo = getConfigItemByKey("MapInfo","q_map_id",map_id)
        local needTitle = false
        if mapInfo.q_need_item_id then
            needItemId = tonumber(mapInfo.q_need_item_id)
        end
        -- if mapInfo.q_min_title_id then
        --     needTitle = true
        -- end
        if G_MAINSCENE.map_layer:isHideMode() and G_MAINSCENE.mapId ~= 6017 then--行会驻地需要特殊处理
            ...
            return
        elseif mapInfo.xianzhi and 
            tonumber(mapInfo.xianzhi) == 1 and
            needItemId == feixieItemId and
            needTitle == false then
            TIPS( getConfigItemByKeys("clientmsg",{"sth","mid" } , { 17000 , -7 } ) )
            return
        ...
        if num <= 0 then
            if needItemId == feixieItemId and needTitle == false then
                isHaveShoes = false
