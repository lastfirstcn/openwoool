...
function WingAndRidingAdvanceNode:networkHander(buff,msgid)
	local switch = {
		...
		[WING_SC_PROMOTE_RET] = function()
			...
			cclog("WING_SC_PROMOTE_RET ret = "..tostring(ret))
			cclog("WING_SC_PROMOTE_RET bless = "..self.blessValue)
			if ret == true then
		...
		[WING_SC_PROMOTE_CONDITION_FAIL] = function()
			cclog("WING_SC_PROMOTE_CONDITION_FAIL")
			--self:stopAutoAdvance()
		end
		,

		[WING_SC_GET_WING_PRICE_RET] = function()
			cclog("WING_SC_GET_WING_PRICE_RET")
