...
function PingNode:check(netMessage, buff)
	--cclog("PingNode:check "..netMessage)
...
function PingNode:checkNetBadCount()
	cclog("PingNode:checkNetBadCount")
	--cclog("self.netBadCount = "..self.netBadCount)
...
function PingNode:checkHeartSpeed(serverTime)
	local time_scale = cc.Director:getInstance():getScheduler():getTimeScale()
		if time_scale > 0.034 and  time_scale <0.032 then
...
-- --加速器判断方法2
-- function PingNode:checkHeartSpeed(serverTime)
-- 	cclog("serverTime = "..serverTime)
-- 	cclog("self.heartLastServerTime = "..tostring(self.heartLastServerTime))
...
-- 		--cclog("localTimeChange = "..localTimeChange)
-- 		--cclog("netTimeChange = "..netTimeChange)
