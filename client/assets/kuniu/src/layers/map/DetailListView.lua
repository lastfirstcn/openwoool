...
function DetailListView:ctor(parent, size, pos, type, tab)
	if parent == nil then
		cclog("DetailListView param error")
		return
	end
	cclog("#tab="..#tab)
	self.data = {}
...
function DetailListView:tableCellTouched(table, cell)
	cclog("DetailListView:tableCellTouched")
	cclog("self.type="..self.type)
...
function DetailListView:tableCellAtIndex(table, idx)
	--cclog("DetailListView:tableCellAtIndex idx:"..idx)
...
function DetailListView:numberOfCellsInTableView(table)
	--cclog("#self.dataTab"..#self.dataTab)
   	return #self.dataTab
end
