    ...
    if self.mapNode ~= nil then
        ...
        listenner:registerScriptHandler(function(touch, event)
                ...
                if cc.rectContainsPoint(self.mapNode:getBoundingBox(), touchPos) then
				    ...
				    cclog("posEnd.x"..posEnd.x)
				    cclog("posEnd.y"..posEnd.y)
...
function DetailMapNode:convertMapPos(src_pos) 
	--cclog("convertMapPos")
	--cclog("src_pos.x"..src_pos.x)
	--cclog("src_pos.y"..src_pos.y)
...
function DetailMapNode:createMap(mapId)
 ...
 --   			--local imageSpr = cc.Sprite:create(image)
	...
	-- cclog("nodeSizeWidth = "..nodeSizeWidth)
	-- cclog("nodeSizeHeight = "..nodeSizeHeight)
	...
	-- cclog("self.mapScale"..self.mapScale)
	...
	cclog("DetailMapLayer:createMap")
...
function DetailMapNode:createMapInfo()
	...
	--cclog("mapInfoNode x = "..307-self.mapNode:getContentSize().width/2)
	--cclog("mapInfoNode y = "..330-self.mapNode:getContentSize().height/2)
	self:addMapInfo()
end
...
function DetailMapNode:addNpcInfo()
	cclog("DetailMapLayer:addNpcInfo")
...
function DetailMapNode:addMonsterInfo()
	cclog("DetailMapLayer:addMonsterInfo")
	...
	for k,v in pairs(self.monsterDataTab) do
		...
		-- cclog("x = "..v.x*self.tileSize.width*self.mapScale)
		-- cclog("y = "..v.y*self.tileSize.height*self.mapScale)
...
function DetailMapNode:addHotInfo()
	cclog("DetailMapLayer:addHotInfo")
...
function DetailMapNode:updateFbMonsterInfo()
	cclog("DetailMapLayer:updateMonsterInfo")
...
-- 		cclog("posStart.x = "..posStart.x.." self.pathEnd.x"..self.pathEnd.x)
-- 		cclog("posStart.y = "..posStart.y.." self.pathEnd.y"..self.pathEnd.y)
...
function DetailMapNode:addPath()
	...
	if self.path == nil or self.pathEnd == nil then
		...
		--cclog("error:path info error !!!!!!!!")
		return
	end

	if not G_ROLE_MAIN then
		--cclog("error: G_ROLE_MAIN is nil !!!!!!!!")
		return 
	end
	...
	if math.abs(posNow.x - self.pathEnd.x) <=3 and math.abs(posNow.y - self.pathEnd.y) <=3 then
		...
		--cclog("error:no self.path or self.pathEnd")
		return
	end 
	...
	if self.pathEnd then
		...
		--cclog("error:no pathEnd")
	end
end

function DetailMapNode:addPoint(choseType, idx)
	--cclog("DetailMapLayer:addPoint")
	--cclog("choseType = ".. choseType)
	--cclog("idx = ".. idx)
...
function DetailMapNode:goToSomeOne(choseType, idx)
	--cclog("DetailMapLayer:goToSomeOne")
	--cclog("choseType = ".. choseType)
	--cclog("idx = ".. idx)
	local pos
	-- if self.point == nil then
...
function DetailMapNode:goTo(pos,map_id,isnear)
	...
	self.pathEnd = posEnd
	--cclog("33333333333333333333333333333333")
...
function DetailMapNode:networkHander(buff,msgid)
	local switch = {
		[TEAM_SC_GETTEAMPOSINFO] = function() 
			cclog("get TEAM_SC_GETTEAMPOSINFO"..msgid)
