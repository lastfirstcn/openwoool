...
function TutoCompulsiveLayer:ctor(showNodeList, touchNodeList, tutoInfo, isHalfCompulsive)
	cclog("TutoCompulsiveLayer:ctor")
	...
	if tutoInfo.q_controls[tutoInfo.q_step].touchNode ~= nil then
		...
		local touchPos
		--触摸node的信息处理
		cclog("tutoInfo.q_id = "..tutoInfo.q_id)
		cclog("tutoInfo.q_controls[tutoInfo.q_step].touchNode = "..tutoInfo.q_controls[tutoInfo.q_step].touchNode)
		...
		if node then
			...
			-- end
			-- cclog(tutoInfo.q_id .." set TUTO_STATE_FINISH")
	  --       tutoSetState(tutoInfo, TUTO_STATE_FINISH)
	        
			removeFromParent(self)
			return
		end
		cclog("touchPos.x =".. touchPos.x)
		cclog("touchPos.y =".. touchPos.y)
	    ...
	    listenner:registerScriptHandler(function(touch, event)
	            ...
	            if cc.rectContainsPoint(node:getBoundingBox(), cc.p(location.x, location.y)) 
	            	...
	            	cclog("tutoInfo.q_step =".. tutoInfo.q_step)
	...
	cclog("delay = "..tostring(delay))
    if delay then
    	cclog("delay scheduler")
