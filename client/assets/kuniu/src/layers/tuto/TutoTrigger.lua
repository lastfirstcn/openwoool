...
function TutoTrigger:ctor()
	...
	local function eventCallback(eventType)
		cclog("eventCallback event = "..eventType)
    ...
    local checkInMain = function()
		...
		for k,v in pairs(G_TUTO_DATA) do
			...
			if v.q_state == TUTO_STATE_OFF then
				...
				-- 		--cclog("TutoTrigger:checkInMain 5 q_id = "..v.q_id)
...
function TutoTrigger:checkCfgData()
	--cclog("TutoTrigger:checkCfgData 1111111111111111111111")
	...
	local function setFinish(record)
		--cclog("setFinish 1111111111111111111111111111")
		tutoSetState(record, TUTO_STATE_FINISH)
	end

	local function reset(record)
		--cclog("reset 111111111111111111111111111111111111")
		record.q_step = 1
	...
	for k,v in pairs(G_TUTO_DATA) do
		local record = v
		--cclog("test 1 k="..tostring(k))
		--cclog("test 1 q_state="..tostring(record.q_state))
...
function TutoTrigger:isCanTriggerNew(id)
	for k,v in pairs(G_TUTO_DATA) do
		...
		-- 	cclog("TutoTrigger:isCanTriggerNew is false id = "..v.q_id)
		-- 	return false
		-- end
...
function TutoTrigger:setShowNode(node, tag)
	cclog("TutoTrigger:setShowNode tag = "..tag)
...
function TutoTrigger:setShowNodeEx(tag)
	cclog("TutoTrigger:setShowNodeEx tag = "..tag)
...
function TutoTrigger:checkCondition(condition, id)
	--cclog("TutoTrigger:checkCondition")
	...
	for k,v in pairs(condition) do
		...
		elseif k == "lv" then
			...
			if lv ~= v then
				result = false
				--cclog("TutoTrigger:checkCondition lv false")
			end
		elseif k == "school" then
			...
			if school ~= v then
				result = false
				--cclog("TutoTrigger:checkCondition school false")
			end
		elseif k == "battle" then
			...
			if battle < v then
				result = false
				--cclog("TutoTrigger:checkCondition battle false")
			end
		elseif k == "task" then 
			result = false
			local task = -1
...
function TutoTrigger:trigger(tutoInfo, isMainScene)
	...
	if G_MAINSCENE then
		...
		--if game.getAutoStatus() == AUTO_ATTACK then --tutoInfo.stopHang == true and 
			--cclog("G_MAINSCENE.map_layer:resetHangup() 111111111111111111111111111111111111111111")
...
function TutoTrigger:closeMenu(tutoInfo)
	cclog("TutoTrigger:closeMenu")
	if tutoInfo.q_controls[tutoInfo.q_step].closeMenu == true then
		cclog("1111111111111111111111111111")
		G_MAINSCENE:setFullShortNode(false)
...
function TutoTrigger:isNodeRunningAction(tutoInfo)
	cclog("TutoTrigger:isNodeRunningAction")
...
function TutoTrigger:checkIsDress(dressId)
	--cclog("TutoTrigger:checkIsDress dressId = "..dressId)
