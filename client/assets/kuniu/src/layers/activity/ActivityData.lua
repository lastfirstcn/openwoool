...
function DATA_Activity:__refreshIconState( buff )
    ...
    -- 首充礼包
    local firstData = {
        lableType = 0,link = "",isActivity = false,leftLabel = 0,activityID = 7,desc = "首充礼包",redState = false,key = "",order = 0,modelID = 112,pic = 0
    }
    DATA_Activity.firstData = firstData
    ...
    for i = 1 , #list do
        ...
        for j = 1 , #data do
            ...
            temp["callback"] = function()
                ...
                local tempJson = json.encode(temp)
                cclog( "活动JSON:" .. tempJson )
                DATA_Activity.CData = temp
                return DATA_Activity:__callback()
            end
            if temp["modelID"] == 1  then    -- 每日签到
