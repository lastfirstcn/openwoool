...
function NewWorldBoss:UpdateByServerData(luabuffer)
    ...
    self.m_bossData[9] = {};
    self.m_bossData[9]["id"] = 6100;
    self.m_bossData[9]["state"] = bossInfo[8].bossLive;           -- // 1 is live, 0 is dead
    self.m_bossData[9]["nextTime"] = bossInfo[8].nextLiveTime;    -- boss 下次复活时间(string)
    self.m_bossData[9]["nextState"] = bossInfo[8].isTomorrow;     -- 0表示不是  1表示是 // 0 is today, 1 is Tomorrow
