...
function ChatView:ctor(index, roleData, tabIndex)
	...
	local function moreBtnFunc()
		...
		param.currSendChannel=self.currSendChannel
	...
	local sendFunc = function(tag,sender)
		...
		cclog("utf8len = "..string.utf8len(text))
		...
		if string.len(text) > 0 then
			...
			if string.sub(text,1,1) == "#" then
				...
				cclog("self.currSendChannel = "..self.currSendChannel)
				--cclog("1 text = "..tostring(text))
				--dump(self.linkTab)
	...
	listenner:registerScriptHandler(function(touch, event)
		local location = self.bg:getParent():convertTouchToNodeSpace(touch)
		-- cclog("location.x =".. location.x)
		-- cclog("location.y =".. location.y)
		local privateChatListView = self:getChildByTag(commConst.TAG_CHAT_PRIVATECHATLISTVIEW)
		if cc.rectContainsPoint(self.bg:getBoundingBox(), cc.p(location.x, location.y)) then
			-- cclog("touch true")
			return true
...
function ChatView:sendLinkNetData(text)
	...
	if linkTab then
		...
		-- 	cclog("v.protoId = "..v.protoId.." v.posIndex = "..v.posIndex.." v.bagId = "..v.bagId.." v.time = "..v.time)
...
function ChatView:updateScrollView()
	...
	-- 	--cclog("reset 11111111111111111111111111111111111111")
	-- 	return
	-- end
	...
	--cclog("self.currDisChannel = "..self.currDisChannel
...
function ChatView:deleteChatById(userId)
	cclog("ChatView:deleteChatById userId = "..tostring(userId))
	for i=1,11 do
		if G_CHAT_INFO[i] then
			...
			for i,v in ipairs(tab) do
				...
				if v.usrId and v.usrId == userId then
					-- cclog("table.remove(tab, i) 1111111111111111111111")
