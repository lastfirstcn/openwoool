...
function ChatMoreLayer:ctor(param)
	...
	self.currSendChannel=param.currSendChannel
	...
	local function coordinatesFunc()
		...
		if isCanSend and isCanSend == 0 then
			...
			--发送坐标
			local t = {}
			t.message = "^a(" .. G_MAINSCENE.mapId .. "," .. line .. "," .. pos.x .. "," .. pos.y .. ")^"
			printf("当前坐标:"..t.message)
			--需要获取当前选择的频道
			t.channel = self.currSendChannel
			printf("当前频道:"..t.channel)
	...
	local x = 40
	local addX = (bg:getContentSize().width - 2 * x) / 4
	local y = bg:getContentSize().height / 2

	local coordinatesBtn = createMenuItem(bg, path.."coordinates.png", cc.p(x+addX*0, y), coordinatesFunc)
	local faceBtn = createMenuItem(bg, path.."face.png", cc.p(x+addX*1, y), faceBtnFunc)
	local equipmentBtn = createMenuItem(bg, path.."equipment.png", cc.p(x+addX*2, y), equipmentBtnFunc)
	local shortBtn = createMenuItem(bg, path.."short.png", cc.p(x+addX*3, y), shortBtnFunc)
	local trumpetBtn = createMenuItem(bg, path.."trumpet.png", cc.p(x+addX*4, y), trumpetFunc)
